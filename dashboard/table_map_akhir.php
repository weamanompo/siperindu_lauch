            ...
            <table class="table table-bordered table-striped" width="60%">
                ...
                <tbody>
                    ...
                    <?php while ($d = mysqli_fetch_assoc($wilayah)) : ?>
                        <?php $kdwil = $d['kode']; ?>
                        <?php $nilai =  mysqli_query($koneksi, "SELECT * FROM transaksi_pilar WHERE kode_wilayah = '$kdwil' AND kd_indi_pilar = '$kd_indi'");
                        ...
                        <tr>
                            ...
                            <td style="text-align: left ;">&nbsp;&nbsp;<a id="kab"><?= $d['nama']; ?></a><input type="text" id="kdwil" value="<?= $kdwil; ?>"><input type="text" id="kdindi" value="<?= $kd_indi; ?>"></td>
                            <!-- <td style="text-align: center;" ><?= $amnilai['nilai_indi_pilar']; ?></td> -->
                            <?php warnatable($n, $kd_indi, $kdwil); ?>
                        </tr>
                        <?php $no++; ?>
                    <?php endwhile; ?>
                    <?php $nasional = mysqli_query($koneksi, "SELECT * FROM transaksi_pilar WHERE kode_wilayah = '1' AND kd_indi_pilar = '$kd_indi'");
...
<script>
    $(document).ready(function() {
        const tes2 = document.querySelectorAll('#kab');
        const kdwil = document.querySelectorAll('#kdwil');
        const kdindi = document.querySelectorAll('#kdindi');

        for (let i = 0; i < tes.length; i++) {

            tes[i].addEventListener('click', function() {

                var provinsi = kdwil[i].value;
                var indikator = kdindi[i].value;
